...
// export const ffmpegVolume = async (
//   fileUrl:string,
//   fileName:string,
//   volumevalue:string
// ) => {
//   ffmpeg.FS('writeFile', fileName, await fetchFile(fileUrl))
//   const cmd = `-i ${fileName} -vf hflip output.mp4`
//   await ffmpeg.run(...cmd.split(' '))
//   const video = ffmpeg.FS('readFile', 'output.mp4')
//   const videoUrl = new Blob([video.buffer], { type: 'video/mp4' })
//   return URL.createObjectURL(videoUrl)
// }
// //变速
// export const ffmpegSpeed = async(
//   fileUrl:string,
//   fileName:string,
//   speedvalue:string
// ) => {
//   ffmpeg.FS('writeFile', fileName, await fetchFile(fileUrl))
//   const command = `-i ${fileName} -vf setpts=PTS/${speedvalue} output.mp4`
//   await ffmpeg.run(...command.split(' '))
//   const video = ffmpeg.FS('readFile', 'output.mp4')
//   const videoUrl = new Blob([video.buffer], { type: 'video/mp4' })
//   return URL.createObjectURL(videoUrl)
